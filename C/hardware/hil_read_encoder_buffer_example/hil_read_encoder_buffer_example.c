//////////////////////////////////////////////////////////////////
//
// hil_read_encoder_buffer_example.c - C file
//
// This example reads 20 samples from encoder input channels 0 through 3,
...
#include "hil_read_encoder_buffer_example.h"
...
int main(int argc, char* argv[])
{
    ...
    if (result == 0)
    {
        ...
        static t_int32 counts[SAMPLES][NUM_CHANNELS];
        t_int          samples_read;

        samples_read = hil_read_encoder_buffer(board, SYSTEM_CLOCK_1, frequency, SAMPLES, channels, NUM_CHANNELS, &counts[0][0]);
        if (samples_read > 0)
        {
            ...
            for (index = 0; index < SAMPLES; index++)
            {
                for (channel = 0; channel < NUM_CHANNELS; channel++)
                    printf("ENC #%d: %5d   ", channels[channel], counts[index][channel]);
                printf("\n");
            }
        }
        else
